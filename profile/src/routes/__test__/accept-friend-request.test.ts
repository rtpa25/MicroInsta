...
it('accepts a friend request with valid inputs', async () => {
    ...
    const response2 = await request(app)
        .put(`/api/profile/accept-friend-request/${userID1}`)
    ...
    expect(profile2Updated!.friends[0].toJSON()).toEqual(profile1Updated!.id);
    expect(profile1Updated!.friends[0].toJSON()).toEqual(profile2Updated!.id);
    expect(response2.body.friends[0]).toEqual(profile2Updated?.id);
});
